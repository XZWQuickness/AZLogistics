    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        switch (list.get(position).getIsCheck()) {//"isCheck": "1",(1:审核通过  2：审核未通过 3：审核中)
            ...
            case "1"://1:审核通过
                ...
                viewHodler.iv_check_car.setBackgroundResource(R.drawable.delete_car);
                viewHodler.tv_check.setText("车辆行驶证证已通过平台审核");
                break;

            case "2": //2：审核未通过
                viewHodler.iv_check.setBackgroundResource(R.drawable.weitongguo);
                viewHodler.iv_check_car.setBackgroundResource(R.drawable.cheliangbianji);
                viewHodler.tv_check.setText("车辆行驶证证未通过平台审核");
                break;

            case "0"://3：审核
                viewHodler.iv_check.setBackgroundResource(R.drawable.shenhezhong);
                viewHodler.iv_check_car.setBackgroundResource(R.drawable.cheliangbianji);
